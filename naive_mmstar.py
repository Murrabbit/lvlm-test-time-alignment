...
from naive_utils import AutoregressiveSampler, SYSTEM_PROMPT, mcmc_power_samp
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(args.save_dir, "mmstar", method_str)
    ...
    for data_idx in tqdm(range(start_idx, end_idx), desc="Processing samples"):
        ...
        try:
            ...
            # Store the result, replacing a failed entry when resuming.
            if not skip_error and (data_idx - start_idx) < cache_size:
                assert results[data_idx - start_idx]['id'] == data['id']
                results[data_idx - start_idx] = {
                    "id": data['id'],
                    "question": question,
                ...
                results_pkl[data_idx - start_idx] = {
                    "id": data['id'],
                    "mcmc_ids": mcmc_ids,
            ...
            if not skip_error and (data_idx - start_idx) < cache_size:
                assert results[data_idx - start_idx]['id'] == data['id']
                results[data_idx - start_idx] = {
                    "id": data.get('id', f"unknown_{data_idx}"),
                    "status": "error",
                    "error": str(e),
            ...
            # Save incrementally to support long-running jobs.
            save_results(save_path, save_pkl, results, results_pkl)
            # Release cached GPU memory between samples.
