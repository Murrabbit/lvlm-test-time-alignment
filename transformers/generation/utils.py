...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    results_pack: Optional[Tuple[torch.FloatTensor]] = None
...
class GenerationMixin:
    ...
    def _get_stopping_criteria(
        ...
        **kwargs,
        ...
        if generation_config._eos_token_tensor is not None:
            criteria.append(EosTokenCriteria(eos_token_id=generation_config._eos_token_tensor, blink_block=generation_config.blink_block))
        if (
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        cross_attentions = () if (return_dict_in_generate and output_attentions) else None
        decoder_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None
        results_packs = [() for _ in range(input_ids.shape[0] // 2)]
        end_flag = [False for _ in range(input_ids.shape[0] // 2)]
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # odd_indices = torch.arange(1, model_inputs['input_ids'].size(0), 2, device=model_inputs['input_ids'].device)
            # mask_to_zero = (model_inputs['input_ids'][odd_indices] == 151655).long()
            # extra_mask = torch.ones_like(model_inputs['attention_mask'])
            # extra_mask[odd_indices] = 1 - mask_to_zero  # 151655 → 0，其余 → 1
            # model_inputs['attention_mask'] = model_inputs['attention_mask'] * extra_mask
            ...
            # 2. 计算熵 H = -sum(p * log p)
            H = -(probs * torch.log(probs + 1e-12)).sum(dim=-1)  # shape: [batch_size]
            H_with_img = H[::2].tolist()
            H_no_img = H[1::2].tolist()
            TE = [h_no - h_with for h_no, h_with in zip(H_no_img, H_with_img)]
            ...
            if do_sample:
                ...
                next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
                next_tokens[1::2] = next_tokens[::2]
            else:
                next_tokens = torch.argmax(next_token_scores, dim=-1)
                next_tokens[1::2] = next_tokens[::2]
            
            for i in range(len(results_packs)):
                if not end_flag[i]:
                    results = (
                        next_tokens[2 * i].item(),
                        H_with_img[i],
                        H_no_img[i],
                        TE[i]
                    )
                    results_packs[i] += (results,)
                
                if next_tokens[2 * i].item() == 151645:
                    end_flag[i] = True
