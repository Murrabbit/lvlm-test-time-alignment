...
class EosTokenCriteria(StoppingCriteria):
    ...
    def __init__(self, eos_token_id: Union[int, List[int], torch.Tensor], blink_block:int):
        if not isinstance(eos_token_id, torch.Tensor):
            if isinstance(eos_token_id, int):
                eos_token_id = [eos_token_id]
            eos_token_id = torch.tensor(eos_token_id)
        self.eos_token_id = eos_token_id
        self.blink_block = blink_block
        
        # 预计算是否包含特殊 token 14374 (避免每次调用都检查)
        self.contains_special_eos = (self.eos_token_id == 14374).any().item()
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> torch.BoolTensor:
        ...
        # 情况1: 不包含特殊结束符 14374 -> 使用标准逻辑
        if not self.contains_special_eos:
            last_tokens = input_ids[:, -1]
            return isin_mps_friendly(last_tokens, self.eos_token_id)
        
        # 情况2: 包含特殊结束符 14374 -> 需要额外检查出现次数
        batch_size = input_ids.size(0)
        is_done = torch.zeros(batch_size, dtype=torch.bool, device=input_ids.device)
        
        # 步骤1: 检查哪些序列的最后一个 token 是 14374
        last_tokens = input_ids[:, -1]
        is_special_end = (last_tokens == 14374)
        
        # 步骤2: 检查哪些序列的最后一个 token 在普通 eos_token_id 中
        is_normal_end = isin_mps_friendly(last_tokens, self.eos_token_id)
        
        # 情况2.1: 最后一个 token 是普通结束符 (非14374) -> 直接结束
        is_done = is_normal_end & ~is_special_end
        
        # 情况2.2: 最后一个 token 是 14374 -> 检查出现次数
        if is_special_end.any():
            # 创建 mask 标记所有 14374 的位置
            special_mask = (input_ids == 14374)
            # 计算每个序列中 14374 的总出现次数
            special_counts = special_mask.sum(dim=1)
            # 满足次数要求的序列
            meets_count = (special_counts >= self.blink_block)
            # 同时满足: 最后一个 token 是 14374 且 次数达标
            is_done_special = is_special_end & meets_count
            is_done = is_done | is_done_special
